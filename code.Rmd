...
- Mahama refugee camp boundary: hand drawn in the UNHCR GIS tool based on satellite image
...
Calculate normalized sanitary density 
```{r}
...
# Join polygons with grid cells
# facility_grid <- grid %>%
#   st_join(facility_buildings_ranked) 
# 
# facility_grid <- facility_grid %>%
#   mutate(rank = ifelse(!is.na(Facility), 1, 0),
#          Social_Vulnerability_Rank = ifelse(is.na(Facility) | Facility == 0, 0, Social_Vulnerability_Rank),
#          Physical_Vulnerability_Rank = ifelse(is.na(Facility) | Facility == 0, 0, Physical_Vulnerability_Rank)) %>%
#   mutate(Facility = if_else(is.na(Facility), "no facility", Facility))
# 
# mapview(facility_grid)
```
...
Calculate normalized shelter density 
```{r}
...
tm_soc <- tm_shape(soc1_raster) +
  tm_raster(palette = "YlOrRd", title = "Flood Vulnerability Value",  style = "cont", breaks = breaks) +
  tm_layout(legend.show = T, title = "SOC", title.position = c("left", "bottom"), title.size = 1, title.color = "black", 
            frame = FALSE) +   tm_legend(outside = TRUE) + 
  tm_shape(mahama_boundary) +
  tm_borders()
...
summary(tot_f_vul_raster)

# create possible realizations from the joint distribution of OC and TN
# sample <- genSample(UMobject = mySpatialMUM, n = 3, samplemethod = "ugs", nmax = 20, asList = FALSE)
```
...
# breaks <- quantile(values(tot_vul_raster), probs = seq(0, 1, by = 0.2))
# 
# tmap_mode("plot")
# 
# tm_vul <- tm_shape(tot_vul_raster) +
...
#             title = "AHP", 
#             breaks = breaks) +  # Set quantile breaks
#   tm_layout(legend.show = TRUE,
#             legend.position = c("left", "bottom"),
#             frame = FALSE) +
#   tm_shape(mahama_boundary) +
...
# 
# # Calculate quantile breaks
# breaks_f <- quantile(values(tot_f_vul_raster), probs = seq(0, 1, by = 0.2))
# 
# # Round breaks to ensure uniqueness
# breaks_f <- round(breaks_f, digits = 3)
# 
# tm_f_vul <- tm_shape(tot_f_vul_raster) +
#   tm_raster(palette = "viridis",
#             title = "FAHP", 
#             breaks = breaks_f) +  # Set quantile breaks
#   tm_layout(legend.show = TRUE,
#             legend.position = c("left", "bottom"),
#             frame = FALSE) +
#   tm_shape(mahama_boundary) +
...
# Arrange tmaps
#tmap_arrange(tm_vul, tm_f_vul, nrow = 1)
```
...
combined_df <- left_join(hfvi_df, uncertainty_df, by = c("x", "y"))
...
# Load your raster data
# Assuming tot_f_vul_raster_smoothed and smoothed_overlay_unc are already loaded in your environment
...
# Convert clipped sf objects back to raster
hfvi_raster_clipped <- rasterFromXYZ(st_coordinates(hfvi_clipped) %>%
                                     as.data.frame() %>%
                                     mutate(HFVI = hfvi_clipped$HFVI))

uncertainty_raster_clipped <- rasterFromXYZ(st_coordinates(uncertainty_clipped) %>%
                                            as.data.frame() %>%
                                            mutate(Uncertainty = uncertainty_clipped$Uncertainty))

# Create the tmap
tm_unc_smoothed <- tm_shape(osm_map) + 
  tm_rgb(alpha = 0.3) +  # Adjust transparency of the OSM map
  tm_layout(legend.position = c("left", "bottom"), frame = FALSE) +  # Adjust legend position and remove frame
  tm_shape(hfvi_raster_clipped) +
  tm_raster(palette = "YlOrRd", 
            style = "cont", 
            title = "HFVI Value", 
            alpha = 0.6, 
            legend.hist = TRUE) + 
  tm_shape(uncertainty_raster_clipped) +
  ...
  tm_layout(title = "HFVI Weight Uncertainty Map - Smoothed",  # Add title
            ...
            legend.title.size = 0.9,
            legend.text.color = "black",
            legend.position = c("left", "bottom"))  # Adjust legend colors and position
...
- Uncertainties (attribute and locational)
- improve uncertainty score: e.g. shelter type when information is not available


Verteilung der Daten:
-> Quartiles anschauen für die einfärbung: jenks, quartiles und nicht equal intervals 

Histogramm von Werten zeigen 

smooting filter anwenden 
