...
```{r Read data}
# AHP results: Data set contain all AHP questionnaire answers (rows) for each attribute 
data_SOC <- read_excel("~/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Uni/Masterarbeit/NatrualHazards_UNHCR/Academic/Documents/05_IndexDevelopment/QuestionnaireAHP/Results/AHP_results.xlsx", 
                       sheet = "SOC", 
                       col_types = c("skip", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric"))
data_SOC <- na.omit(data_SOC)

data_PHY <- read_excel("~/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Uni/Masterarbeit/NatrualHazards_UNHCR/Academic/Documents/05_IndexDevelopment/QuestionnaireAHP/Results/AHP_results.xlsx", 
                       sheet = "PHY", 
                       col_types = c("skip", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric"))
data_PHY <- na.omit(data_PHY)

data_DOM <- read_excel("~/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Uni/Masterarbeit/NatrualHazards_UNHCR/Academic/Documents/05_IndexDevelopment/QuestionnaireAHP/Results/AHP_results.xlsx", 
                       sheet = "Domain", 
                       col_types = c("skip", "numeric", "numeric", "numeric"))
data_DOM <- na.omit(data_DOM)
```
...
ggsave("figs/ind_w_soc.png", ind_w_soc, width = 12, height = 4)
ggsave("figs/ind_w_phy.png", ind_w_phy, width = 12, height = 4)

# Arrange the two plots vertically
ind_w <- grid.arrange(ind_w_soc, ind_w_phy, ncol = 2)

ggsave("figs/ind_w.png", ind_w, width = 10, height = 5)
```
...
ggsave("figs/dom_w.png", dom_w, width = 5, height = 5)
```
...
```{r save plots}
# Save plots to the "figs" folder
ggsave("figs/top_error_soc.png", top_error_soc, width = 12, height = 4)
ggsave("figs/top_error_phy.png", top_error_phy, width = 12, height = 4)

# Arrange the two plots vertically
ind_w <- grid.arrange(top_error_soc, top_error_phy, ncol = 2)

ggsave("figs/top_error.png", ind_w, width = 10, height = 5)
```
...
```{r save plots}
# Save plots to the "figs" folder
ggsave("figs/harker_soc.png", harker_soc, width = 12, height = 4)
ggsave("figs/harker_phy.png", harker_phy, width = 12, height = 4)
ggsave("figs/harker_dom.png", harker_dom, width = 12, height = 4)

# Arrange the two plots vertically
harker_it <- grid.arrange(harker_soc, harker_phy, ncol = 2)

ggsave("figs/harker_it.png", harker_it, width = 10, height = 5)
```
...
```{r Create data frame only containing consistent PCMs, from the participants ID with Consistency = 0}
...
Filter out still inconsistent criteria weights!! then aggregation of final weights and fuzzification

# -----------------------------------------
# Canned routine for AHP analysis 

```{r Canned routine }
canned <- ahp(df = as.data.frame(data_SOC),
              atts = atts_soc,
              negconvert = TRUE, 
              reciprocal = TRUE,
              method = 'eigen',
...
canned$indpref %>% rownames_to_column('ID') %>% kable() %>% kable_styling()

# aggregated preferences 
canned$aggpref %>% t() %>% kable() %>% kable_styling()
```
The table summarizes the individual priorities calculated using the arithmetic mean method, the consistency ratio of each individual, and which three pairwise comparisons are the most inconsistent.
